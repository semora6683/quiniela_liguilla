...
admin_key_form = st.sidebar.text_input("🔑 Código de administrador:", type="password", key="admin_key_sidebar")
is_admin = admin_key_form == "admin123"

with tab1:
    st.header("📝 Participa llenando tu Quiniela")
    if is_admin and os.path.exists(PRED_FILE):
        with st.expander("📥 Descargar predicciones (solo admin)"):
            pred_df = pd.read_csv(PRED_FILE)
            st.dataframe(pred_df)
            csv = pred_df.to_csv(index=False).encode("utf-8")
            st.download_button("📤 Descargar CSV", csv, "predicciones_capturadas.csv", "text/csv")
    with st.form("form_predicciones", clear_on_submit=True):
        ...
        if puede_editar_ida:
            with st.expander("📅 Partidos de IDA (hasta 7 mayo 6:55 PM CDMX)"):
                for partido in ida:
                    ...
                    with col1:
                        local = st.number_input(f"{partido} - Goles Local", min_value=0, max_value=20, key=f"ida-{partido}-L")
                    with col2:
                        visita = st.number_input(f"{partido} - Goles Visitante", min_value=0, max_value=20, key=f"ida-{partido}-V")
        ...
        if puede_editar_vuelta:
            with st.expander("📅 Partidos de VUELTA (hasta 10 mayo 5:00 PM CDMX)"):
                for partido in vuelta:
                    col1, col2 = st.columns(2)
                    with col1:
                        local = st.number_input(f"{partido} - Goles Local", min_value=0, max_value=20, key=f"vuelta-{partido}-L")
                    with col2:
                        visita = st.number_input(f"{partido} - Goles Visitante", min_value=0, max_value=20, key=f"vuelta-{partido}-V")
                    predicciones[f"{partido} - Local"] = local
                    predicciones[f"{partido} - Visitante"] = visita
        else:
        ...
        if submitted:
            if not participante.strip():
                ...
                row["Timestamp"] = ahora.strftime("%Y-%m-%d %H:%M:%S")
...
with tab2:
    ...
    admin_key = st.text_input("🔑 Código de administrador:", type="password")
